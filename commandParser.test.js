...
test.each([
    ...
    ['$12abc'],
    ['$']
])('parseCommand handles an unrecognised command', (message) => {
    const result = parseCommand(message);

    expect(result.data).toHaveProperty('description', "Command not recognised.");
});

test('parseCommand handles a resistance roll', () => {
    Math.random
        .mockReturnValueOnce(0.5) // 4
    ...
    expect(result.data).toHaveProperty('description', '4 from **4** 2 \nTake 2 stress!');
});

test('parseCommand handles a resistance roll too high in dice', () => {
    Math.random
        .mockReturnValueOnce(0.5) // 4
        .mockReturnValueOnce(0.2); // 2
    ...
    expect(result.data).toHaveProperty('description', "You can't resist with more than 4 dice.");
});
...
test.each([
    ...
    ['$r12abc'],
    ['$r'],
])('parseCommand handles a malformed resistance roll command', (message) => {
    const result = parseCommand(message);
